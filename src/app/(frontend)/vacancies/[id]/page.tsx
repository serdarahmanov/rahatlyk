...
export default async function VacancyDetailPage({ params }: Props) {
  ...
  const [vacancyResult, othersResult, formsRaw] = await Promise.allSettled([
    payload.find({
      collection: 'vacancies',
      depth: 2,
      locale,
      limit: 1,
      where: { id: { equals: vacancyID } },
    }),
    payload.find({
      collection: 'vacancies',
      depth: 2,
      ...
      limit: 3,
      sort: '-postedDate',
      where: { id: { not_equals: vacancyID } },
    }),
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (payload.findGlobal as any)({ slug: 'forms', locale, depth: 0 }),
  ])

  const vacancy = vacancyResult.status === 'fulfilled' ? vacancyResult.value.docs[0] : null
  if (!vacancy) notFound()

  const others = othersResult.status === 'fulfilled' ? othersResult.value.docs : []
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const f: any = formsRaw.status === 'fulfilled' ? formsRaw.value : null
